...
import numpy as np
import dl.model.fastspeech.hparams as hp
import os
...
from dl.model.fastspeech.fastspeech2 import FastSpeech2
from dl.model.fastspeech.vocoder import vocgan_generator

from dl.model.fastspeech.text import text_to_sequence, sequence_to_text
import dl.model.fastspeech.utils as utils
import dl.model.fastspeech.audio as Audio
...
#     checkpoint_path = os.path.join(hp.checkpoint_path, "checkpoint_{}.pth.tar".format(num))
    checkpoint_path = 'dl/model/fastspeech/ckpt/kss/checkpoint_550000.pth.tar'
...
#     mean_energy, std_energy = torch.tensor(np.load(os.path.join(hp.preprocessed_path, "energy_stat.npy")), dtype=torch.float).to(device)
    mean_mel, std_mel = torch.tensor(np.load("dl/model/fastspeech/preprocessed/kss/mel_stat.npy"), dtype=torch.float).to(device)
    mean_f0, std_f0 = torch.tensor(np.load("dl/model/fastspeech/preprocessed/kss/f0_stat.npy"), dtype=torch.float).to(device)
    mean_energy, std_energy = torch.tensor(np.load("dl/model/fastspeech/preprocessed/kss/energy_stat.npy"), dtype=torch.float).to(device)
...
def synthesize_voice(text, user_id, we_id):
    ...
    if mode != '4':
        ...
        synthesize(model, vocoder, text, sentence, user_id, we_id, prefix='step_{}'.format(args.step))
